...
public ResponseEntity<String> aprobarDesaprobarCategoria(Integer idSolicitud, boolean aprobar) {
    try {
        ...
        if ("aprobado".equals(estadoActual)) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                    .body("No se puede aprobar una solicitud que ya ha sido aprobada.");
        }

        if ("rechazado".equals(estadoActual)) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                    .body("La solicitud ya ha sido rechazada anteriormente.");
        }
        ...
        if (idUsuarioInstructor == null) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                    .body("No se pudo obtener el usuario instructor.");
        }
  ...
  // Llamada con el motivo de rechazo
        }
        ...
        if (filasAfectadas > 0) {
            ...
            return ResponseEntity.ok("Solicitud " + (aprobar ? "aprobada y categoría creada" : "rechazada") + " con éxito.");
        } else {
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body("Error al actualizar el estado de la solicitud.");
        }
    ...
    private void enviarCorreoNotificacion(String destinatario, String asunto, String motivoRechazo, boolean aprobado, String nombreCategoria) {
        try {
            ...
            if (aprobado) {
                cuerpoMensaje += "<p>¡Felicidades! Su solicitud para la categoría <strong>" + nombreCategoria + "</strong> ha sido aprobada.</p>";
            } else {
                cuerpoMensaje += "<p>Lamentamos informarle que su solicitud para la categoría <strong>" + nombreCategoria + "</strong> ha sido rechazada.</p>" +
